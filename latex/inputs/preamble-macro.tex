...
\newcommand{\titel}{Boek van de MOOC}

\newcommand{\filenaamlogo}{KULEUVEN_CMYK_LOGO}
...
\newcommand*\runningheader[1]{\settowidth{\evenlengte}{\mbox{\bf #1}}\addtolength{\evenlengte}{6mm}%
  ...
  \begin{tikzpicture}
    ...
    \node[xshift=1.5cm,yshift=5.5mm,anchor=south west]{\color{white}\fontsize{11pt}{13pt}\selectfont\bf Wiskunde voor (startende) studenten - KU Leuven};
...
\newcommand{\stellingnaam}[1]{{\tikzset{external/export=false}\begin{tikzpicture}[baseline=(X.base)] \node[inner sep=0.6mm,rectangle,fill=fac-color-dark](X){\color{white}#1}; \end{tikzpicture}}}
...
\newcommand{\voorbeeldnaam}[1]{{\tikzset{external/export=false}\begin{tikzpicture}[baseline=(X.base)] \node[inner sep=0.6mm,rectangle,fill=fac-color-orange](X){\color{white}#1}; \end{tikzpicture}}}
...
{\theoremstyle{mijnopmerking}
	\newtheorem*{rekenregel}{Rekenregel}}

\newenvironment{ftrekenregel}
{\begin{mdframed}\begin{rekenregel}}
		{\end{rekenregel}\end{mdframed}}
...
\newenvironment{tabel*}[1]{\begin{center}\refstepcounter{tabelnr}\renewcommand{\tabeltitel}{#1}\end{center}}
